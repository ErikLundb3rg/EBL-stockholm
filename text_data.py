...
def text_data(transcription = None):
    ...
    dataObject = {
        ...
        "medianWPM": sorted([x['value'] for x in get_wpm_array(transcription)])[len(get_wpm_array(transcription)) // 2],
        "averagePause": sum([x['value'] for x in pauseLengths]) / len(pauseLengths),
        "medianPause": sorted([x['value'] for x in pauseLengths])[len(pauseLengths) // 2],
